...
我們採用 Docker Compose / Kubernetes 進行微服務編排，確保系統的高可用性與安全性：
### 1. 容器組成 (Container Services)
* frontend-ui (SolidStart)：用戶交互界面，透過 WebSocket 即時顯示 Agent 在 Workspace 的動作。
* api-gateway (FastAPI)：處理認證、任務派發，並管理 Workspace 的存取權限。
* agent-executor (Python/CrewAI)：核心邏輯容器，動態啟動多個 Worker 來執行 Agent 任務。
* sandbox-runtime (Python/Node)：隔離執行環境。當 Agent 需要執行 Code 或處理大型文件時，在一個完全沒有網絡權限的臨時容器中運行。
* redis-queue：處理非同步任務隊列（Celery），確保 Agent 在背景長時間工作時，平台不會崩潰。
* vector-db (Milvus/Qdrant)：儲存各個 Agent 的長期記憶與 Workspace 的文件索引。
